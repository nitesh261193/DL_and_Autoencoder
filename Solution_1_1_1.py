...
def model(X_train, Y_train, X_val, Y_val, X_test, Y_test, hidden_nodes, num_Iterations):
    variables = {}
    ...
    layer_1_weights = tf.Variable(tf.random.normal(shape=(input_nodes, hidden_nodes), mean=0.0, stddev=0.05))

    # initialise weights going from hidden_nodes into output_node
    layer_2_weights = tf.Variable(tf.random.normal(shape=(hidden_nodes, output_nodes), mean=0.0, stddev=0.05))

    # initialise bias weights going into hidden_nodes
    layer_1_bias = tf.Variable(tf.random.normal(shape=(1, hidden_nodes)))

    # initialise bias weights going into output_node
    layer_2_bias = tf.Variable(tf.random.normal(shape=(1, output_nodes)))
